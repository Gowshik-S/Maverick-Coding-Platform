...
export default function ProfileBuilder() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(response?.error || response?.detail || 'Registration failed.');
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-surface text-on-surface font-body antialiased selection:bg-primary-container selection:text-on-primary-container">
      <nav className="fixed top-0 w-full z-50 bg-[#1b1b1c] flex justify-between items-center h-14 px-6 max-w-full">
        ...
        <div className="flex items-center gap-4">
          <button type="button" className="text-[#dac2b2] hover:text-[#e5e2e1] transition-colors text-sm font-medium px-4 py-2">Log In</button>
          <button type="button" className="bg-primary-container text-on-primary-container px-4 py-1.5 rounded-sm text-sm font-bold active:scale-95 transition-transform">Sign Up</button>
        </div>
      </nav>
        ...
        <section className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 bg-surface-container-low">
          <div className="w-full max-w-2xl space-y-8">
            ...
            <form onSubmit={onSubmit} className="space-y-8">
              ...
              <label htmlFor="resume-upload" className="custom-dashed group hover:bg-surface-container transition-all duration-300 rounded-xl p-12 flex flex-col items-center justify-center text-center cursor-pointer min-h-[320px]">
                ...
                <span className="bg-surface-variant hover:bg-surface-bright text-on-surface px-6 py-2.5 rounded-sm font-semibold text-sm border border-outline-variant/30 transition-colors">
                  Browse Files
                </span>
              </label>

              {file && (
                <p className="text-sm text-on-surface-variant">Selected: <span className="text-on-surface font-semibold">{file.name}</span></p>
              )}
              ...
              <div className="text-center">
                <Link to="/onboarding/manual" className="text-tertiary hover:underline text-sm font-medium flex items-center justify-center gap-2 group">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-8">
                <Link to="/register" className="w-full md:w-auto order-2 md:order-1 flex items-center justify-center gap-2 text-on-surface-variant hover:text-on-surface transition-colors px-6 py-3 font-bold text-sm">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full md:w-auto order-1 md:order-2 bg-primary-container hover:brightness-110 text-on-primary-container px-10 py-3.5 rounded-sm font-extrabold text-sm tracking-wide shadow-xl shadow-primary-container/10 flex items-center justify-center gap-2 active:scale-95 transition-all disabled:opacity-60"
                >
