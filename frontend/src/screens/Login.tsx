...
export default function Login({ onNavigate, onNotify }: LoginProps) {
  ...
  const submit: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    try {
      ...
      if (!data.user_id) {
        onNotify?.(data.error || data.detail || 'Login failed');
        return;
      }
      saveSession({
      ...
      onNotify?.(err.message || 'Login failed');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="w-full md:w-7/12 bg-[#131313] p-12 md:p-20 flex flex-col justify-center border-l border-[#353535]">
          <div className="max-w-md w-full mx-auto">
            ...
            <form className="space-y-5" onSubmit={submit}>
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input type="email" id="email" className="w-full bg-[#1b1b1c] border border-[#353535] text-[#e5e2e1] text-sm rounded-none focus:ring-1 focus:ring-[#e6883a] focus:border-[#e6883a] block pl-10 p-3 transition-colors" placeholder="engineer@maverick.io" required value={email} onChange={(e) => setEmail(e.target.value)} />
                </div>
              </div>
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <a href="#" onClick={comingSoon} className="text-xs text-[#e6883a] hover:underline font-medium">Forgot password?</a>
                </div>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <span className="material-symbols-outlined text-[#544338] text-lg">lock</span>
                  </div>
                  <input type="password" id="password" className="w-full bg-[#1b1b1c] border border-[#353535] text-[#e5e2e1] text-sm rounded-none focus:ring-1 focus:ring-[#e6883a] focus:border-[#e6883a] block pl-10 p-3 transition-colors" placeholder="••••••••" required value={password} onChange={(e) => setPassword(e.target.value)} />
              ...
              <button type="submit" disabled={loading} className="w-full bg-[#e6883a] hover:bg-[#ffb782] text-[#131313] font-black tracking-wider uppercase text-sm py-4 px-4 rounded-none transition-colors mt-4 flex justify-center items-center gap-2 disabled:opacity-70">
            ...
            <div className="mt-6 grid grid-cols-2 gap-4">
              <button type="button" onClick={() => onNotify?.('Coming soon')} className="flex items-center justify-center gap-2 w-full bg-[#1b1b1c] border border-[#353535] hover:bg-[#353535] text-[#e5e2e1] font-bold text-sm py-3 px-4 rounded-none transition-colors">
            ...
            <p className="mt-8 text-center text-xs text-[#544338]">
              By signing in, you agree to our <a href="#" onClick={comingSoon} className="hover:text-[#dac2b2] underline">Terms of Service</a> and <a href="#" onClick={comingSoon} className="hover:text-[#dac2b2] underline">Privacy Policy</a>.
            </p>
          </div>
        </div>
      </div>
